...
REPO_DATE     ?= $(shell $(GIT) show --oneline --format='%ci' HEAD | head -n1 | cut -d\  -f1)
REPO_MODIFIED ?= $(shell [ "`$(GIT) diff`" == "" ] && echo "" || echo "M")
REPO_BRANCH   ?= $(shell $(GIT) rev-parse --abbrev-ref HEAD | grep -v 'HEAD' | grep -v 'master' | sed "s/\//-/g")
REPO_TAGS     ?= $(shell $(GIT) describe --tags 2>/dev/null | head -n1 || echo "")
REPO_REVISION ?= 42
endif
